...
/* includes for kokkos-resilience */
...
int main(int argc, char *argv[]) {
    ...
    // NEW NAME FOR FILE-BASED API
    auto ctx = KokkosResilience::Context< KokkosResilience::VeloCMemoryBackend >( MPI_COMM_WORLD, "heatdis.cfg");

    wtime = MPI_Wtime();
    int v = KokkosResilience::latest_version(ctx, "test_kokkos");
    if (v > 0) {
        i = v;
    } else {
        i = 0;
    ...
    while(i < ITER_TIMES) {
        ...
        }, KokkosResilience::filter::nth_iteration_filter<CKPT_FREQ>{});
